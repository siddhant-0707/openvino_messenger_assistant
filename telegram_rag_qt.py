...
class ModelConfigPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        info_text.setHtml("""
        <p><b>TextEmbeddingPipeline Integration:</b> This application uses the native 
        <code>openvino_genai.TextEmbeddingPipeline</code> from the 
        <a href="https://github.com/openvinotoolkit/openvino.genai/tree/master/samples/python/rag">official OpenVINO GenAI RAG samples</a>.</p>
        
        <p><b>Key Features:</b></p>
        <ul>
        <li>Native TextEmbeddingPipeline integration</li>
        <li>Multiple implementation options</li>
        <li>Automatic model downloads from <a href="https://huggingface.co/OpenVINO">OpenVINO repositories</a></li>
        </ul>
        """)
...
class TelegramPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        download_group = QGroupBox("Download Messages from Telegram Channels")
        download_layout = QFormLayout(download_group)
        
        self.channels_input = QLineEdit()
        self.channels_input.setPlaceholderText("Enter channel names without @ symbol (e.g., guardian, bloomberg)")
        self.channels_input.setText("guardian,bloomberg")
        download_layout.addRow("Channel Names (comma-separated):", self.channels_input)
        ...
        process_info = QLabel("Convert downloaded messages into searchable vector store")
    ...
    def download_messages(self):
        ...
        if not channels_str:
            self.download_status.setText("Please provide at least one channel name")
            return
        ...
        self.download_requested.emit(channels, limit, hours)
...
class TypingIndicator(QWidget):
    ...
    def update_text(self, text):
        ...
        self.message_text_widget.setPlainText(text)
        ...
        # Style and size the text widget
...
class ChatMessage(QWidget):
    ...
    def setup_ui(self):
        ...
        # Message text with dynamic sizing
        ...
        if self.is_user:
            ...
            sender_label.setStyleSheet("""
                ...
                margin-bottom: 2px;
            """)
            # Override text color for user messages
            message_text.setStyleSheet("""
                QTextBrowser {
                    background-color: transparent;
                    border: none;
            ...
            sender_label.setStyleSheet("""
                ...
                margin-bottom: 2px;
            """)
            # Ensure text is dark for assistant messages
            message_text.setStyleSheet("""
                QTextBrowser {
                    background-color: transparent;
                    border: none;
    ...
    def update_text(self, text):
        ...
        if hasattr(self, 'message_text_widget'):
            ...
            # Reapply styling to ensure text remains visible
            if self.is_user:
                self.message_text_widget.setStyleSheet("""
                    QTextBrowser {
                        background-color: transparent;
...
• "Summarize the latest OpenVINO news and updates"

✨ **Getting Started**
Simply type your question below and I'll search through your processed Telegram messages to provide detailed, contextual answers.
...
class StreamingCallback(QObject):
    """Callback handler for streaming text updates"""
